...
def login():
    if request.method == 'POST':
        ...
        # Replace this helper with your user lookup and password-hash verification.
        if authenticate_user(username, credential):
...
Use an environment variable or secret manager for `SECRET_KEY`. If this form accepts a password, hash and verify it with your authentication layer instead of logging or comparing raw values in source code.
...
app = Flask(__name__)
app.config['SECRET_KEY'] = os.environ['SECRET_KEY']  # Required for CSRF
...
class RegistrationForm(FlaskForm):
    ...
    credential = PasswordField('Password', validators=[
        DataRequired(),
        Length(min=6)
    ])
    credential_confirmation = PasswordField('Confirm Password', validators=[
        DataRequired(),
        EqualTo('credential', message='Entries must match.')
    ])
    submit = SubmitField('Register')
```

When a `PasswordField` is used for real credentials, submit it only over HTTPS and hash it before storage.
...
def register():
    ...
    if form.validate_on_submit():
        ...
        # Process the data (e.g., hash the password, then save to database)
...
<form method="POST" action="{{ url_for('register') }}">
    ...
    <div>
        ...
        {{ form.credential(class="form-control") }}
    ...
    <div>
        {{ form.credential_confirmation.label }}
        {{ form.credential_confirmation(class="form-control") }}
        {% for error in form.credential_confirmation.errors %}
            <span class="error">{{ error }}</span>
        {% endfor %}
    </div>
