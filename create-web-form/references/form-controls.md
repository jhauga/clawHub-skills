...
This example demonstrates layout only. In production, collect passwords only when necessary, hash them server-side, and use hosted payment fields or tokenization from a trusted processor instead of raw card data inputs.
...
<form>
  ...
  <section>
    ...
    <p>
      ...
      <input type="text" id="account-reference" name="account-reference" required />
    </p>
  </section>

  <!-- Payment Information Section -->
  <section>
    <h2>Payment provider information</h2>

    <p>
      <label for="card">Payment method:</label>
      <select id="card" name="payment-method">
    ...
    <p>
      <label for="payment-token">Processor payment token *:</label>
      <input type="text" id="payment-token" name="payment-token" required />
    </p>
  </section>
  ...
  <section>
    <p>
      <button type="submit">Continue to payment provider</button>
    </p>
  </section>
</form>
```
...
```html
<input type="password" id="current-passphrase" name="current-passphrase" />
```

**Security Note:** This only hides the text visually. Only request passwords when necessary, use HTTPS for transmission, and never log or store raw password values.
